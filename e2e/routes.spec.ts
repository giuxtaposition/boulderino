...
const seedSystem = async (
  page: Page,
  name: string,
  grades: { name: string; order: string }[],
): Promise<void> => {
  ...
  for (let i = 0; i < grades.length; i++) {
    ...
    await page.getByTestId(`input-grade-name-${i}`).fill(grades[i].name);
...
test.describe("Routes screen", () => {
  ...
  test("requires name, grading system, grade and photo before submitting", async ({
    ...
    await page.goto("/");
    await seedSystem(page, "YDS", [{ name: "5.10a", order: "1" }]);
    await openRoutesTab(page);
    ...
    await page.getByTestId("submit-route").click();
    await expect(page.getByTestId("error-route")).toContainText("Pick a photo");
  });
