...
const fillGradeRow = async (
  page: Page,
  index: number,
  args: { value: string; label?: string; order?: string; colorKey?: string },
): Promise<void> => {
  await page.getByTestId(`input-grade-value-${index}`).fill(args.value);
  if (args.label !== undefined) {
    await page.getByTestId(`input-grade-label-${index}`).fill(args.label);
  }
...
test.describe('Grading systems screen', () => {
  test('adds a new grading system and shows its colored grades', async ({
    ...
    await expect(
      page.getByTestId('system-grade-YDS-5.10a'),
    ).toContainText('Ten A');
    await expect(
      page.getByTestId('system-grade-YDS-5.10b'),
    ).toContainText('Ten B');
  });
  ...
  test('shows an error when the system name is empty', async ({ page }) => {
    ...
    await fillGradeRow(page, 0, { value: 'V0', order: '1' });
    await page.getByTestId('submit-system').click();
  ...
  test('shows an error when no grade values are provided', async ({ page }) => {
    await openGradesTab(page);
  ...
  test('shows an error when registering a duplicate system', async ({
    ...
    await page.getByTestId('input-system-name').fill('YDS');
    await fillGradeRow(page, 0, { value: '5.10a', order: '1' });
    await page.getByTestId('submit-system').click();
    await expect(page.getByTestId('system-row-YDS')).toBeVisible();

    await page.getByTestId('input-system-name').fill('YDS');
    await fillGradeRow(page, 0, { value: '5.10b', order: '1' });
    await page.getByTestId('submit-system').click();
